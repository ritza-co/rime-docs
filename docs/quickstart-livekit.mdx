...
- An **[OpenAI API Key*](https://platform.openai.com/api-keys)** for speech-to-text (STT) and LLM responses (see [Alternative Providers](#step-6-alternative-providers) for using Deepgram and Gemini instead)
...
Run `pip install -r requirements.txt` to install the new dependencies. This replaces the OpenAI plugin with Deepgram and Google plugins.
...
Get your keys from [console.deepgram.com](https://console.deepgram.com/) and [aistudio.google.com/apikey](https://aistudio.google.com/). Keep your LiveKit and Rime variables unchanged.

### 6.3 Update the agent code

Update the imports in `agent.py`:

```python
from livekit.plugins import deepgram, google, noise_cancellation, rime, silero
```

Update the `AgentSession` configuration:
...
Your agent now uses Deepgram for transcription and Gemini for responses while keeping Rime for TTS.
